...
# Configuration
~/NetChannel/scripts/run_single_flow_set_up_mptcp_0.sh $iface
...
while (( flow < flows ));do
        ((core=16+4*flow))
        ((port=4000+flow))
        sudo taskset -c 28 iperf -c $server_ip -p $port -t 60 > thru.log &
        ((flow++))
done
...
thru=$(grep sec thru.log | awk '{x=x+$7;} END {print x;}')
cpu_server=$(grep Average: cpu_server.log | awk '{x=$3+$5;} END {print x/100.0;}')

echo ""
echo "Throughput: $thru (Gbps)"
echo ""
echo "Receiver-side CPU breakdown:"
python3 ~/NetChannel/scripts/cpu_breakdown_fig2.py perf.log 1.0
cat perf.log_result
