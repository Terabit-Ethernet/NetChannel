...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "46p:svr:w:H:S:zxkP:M:C:a:")) != -1) {
		switch (c) {
		case '4':
			cfg_family = PF_INET;
	...
	setup_sockaddr(cfg_family, server, &listenaddr);
	if (sflg) {
	...
	if (connect(fd, (const struct sockaddr *)&listenaddr, cfg_alen) == -1) {
		perror("connect");
		exit(1);
	}
	if (max_pacing_rate &&
