...
data_cpy = {"csum_and_copy_from_iter_full", "skb_copy_bits", "__memcpy", "memcpy_to_page", "csum_partial_copy_generic", "skb_copy_datagram_iter", "check_stack_object", "copyout", "simple_copy_to_iter", "iov_iter_advance", "__virt_addr_valid", "copy_user_enhanced_fast_string", "memcpy_erms", "__check_object_size",
 "__skb_datagram_iter","_copy_from_iter_full", "_copy_to_iter", "copyin"}
...
"inet_sendmsg", "__sys_sendto", "__x64_sys_sendto", "sockfd_lookup_light", "apparmor_socket_sendmsg"}
...
"tcp_small_queue_check.isra.41", "tcp_tx_timestamp", "bictcp_acked", "tcp_rate_check_app_limited",
"tcp_leave_memory_pressure", "tcp_ack_update_rtt.isra.52", "__tcp_push_pending_frames"}
...
"__alloc_skb", "skb_try_coalesce", "build_skb", "__build_skb" , "__skb_clone", "skb_clone", "skb_csum_hwoffload_help"}
...
"kmem_cache_free_bulk", "get_page_from_freelist", "free_pcppages_bulks","free_pcppages_bulk", "kmem_cache_free", "ksize", "__ksize",
"memcg_kmem_put_cache", "sk_page_frag_refill", "is_vmalloc_addr", "discard_slab", "free_compound_page", "alloc_pages_current", "__sk_mem_raise_allocated", "should_failslab"}
...
"skb_network_protocol", "netdev_pick_tx", "gro_normal_list.part.133", "mlx5_alloc_cmd_msg", "mlx5_eq_async_int"}
...
"native_write_msr", "ktime_get", "__do_softirq", "update_cfs_group","switch_fpu_return", "__update_load_avg_se",
 ...
 "mod_timer", "__libc_send"}
netfilter={"nf_conntrack_tcp_packet", "__nf_conntrack_find_get", "nf_hook_slow", "nf_nat_inet_fn", 
"nf_conntrack_in", "nf_ct_deliver_cached_events", "nf_nat_ipv4_local_fn", "nf_ct_get_tuple", "nf_nat_ipv4_out", "nf_nat_ipv4_fn", "nf_nat_ipv4_in", 
"nf_nat_packet", "nf_confirm", "__nf_ct_refresh_acct", "iptable_filter_hook", "nf_ct_seq_offset"}
...
def parse_contri(filename="../results/nsdi2021/oto/baseline_s"):
    ...
    with open(filename) as f:
        ...
        for i in range(len(lines)):
            ...
            if len(params) < 5:
                continue
            percentage = float(params[0][:-1]) / 100.0
            function = params[4]
            if function in data_cpy:
                p.dc_cost += percentage
            elif function in tcp_ip_p:
                p.tcp_ip_cost += percentage
            elif function in system_call:
                p.system_cost += percentage
            elif function in net_driver:
                p.nd_cost += percentage
            elif function in skb:
                p.skb_cost += percentage
            elif function in memory:
                p.memory_cost += percentage
            elif  function in lock:
                p.lock_cost += percentage
            elif  function in network_sub:
                p.network_sub_cost += percentage 
            elif function in schedule:
                p.schedule_cost += percentage
            elif function in netfilter:
                p.netfilter_cost += percentage
            elif  function in etc:
                # print lines[i]
                # print params
                p.etc_cost += percentage

            else:
                # print (lines[i])
                p.etc_cost += percentage
    output_file(filename + "_result", p)
